...
static int callback_minimal_client_echo(struct lws *wsi, enum lws_callback_reasons reason,
                                        void *user, void *in, size_t len) {
    struct per_session_data__minimal_client_echo *pss =
        (struct per_session_data__minimal_client_echo *)user;
    struct vhd_minimal_client_echo *vhd =
        (struct vhd_minimal_client_echo *)lws_protocol_vh_priv_get(lws_get_vhost(wsi),
    ...
    case LWS_CALLBACK_PROTOCOL_INIT:
        ...
        vhd->interrupted =
            (int *)lws_pvo_search((const struct lws_protocol_vhost_options *)in, "interrupted")
                ->value;
        vhd->port =
            (int *)lws_pvo_search((const struct lws_protocol_vhost_options *)in, "port")->value;
        vhd->options =
            (int *)lws_pvo_search((const struct lws_protocol_vhost_options *)in, "options")->value;
        vhd->ads =
            (const char **)lws_pvo_search((const struct lws_protocol_vhost_options *)in, "ads")
                ->value;
        vhd->url =
            (const char **)lws_pvo_search((const struct lws_protocol_vhost_options *)in, "url")
                ->value;
        vhd->iface =
            (const char **)lws_pvo_search((const struct lws_protocol_vhost_options *)in, "iface")
                ->value;
        vhd->klass =
            (ObsWsClass **)lws_pvo_search((const struct lws_protocol_vhost_options *)in, "klass")
                ->value;
    ...
    case LWS_CALLBACK_CLIENT_WRITEABLE:
        ...
        flags = lws_write_ws_flags(pmsg->binary ? LWS_WRITE_BINARY : LWS_WRITE_TEXT, pmsg->first,
                                   pmsg->final);
    ...
    case LWS_CALLBACK_CLIENT_RECEIVE: {
        ...
        ObsWsClass *klass = *(vhd->klass);

        if (json_parser_load_from_data(parser, (const gchar *)in, len, NULL)) {
...
#define LWS_PLUGIN_PROTOCOL_MINIMAL_CLIENT_ECHO                                                    \
    {                                                                                              \
        "lws-minimal-client-echo", callback_minimal_client_echo,                                   \
            sizeof(struct per_session_data__minimal_client_echo), 1024, 0, NULL, 0                 \
    }